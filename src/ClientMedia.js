...
function ClientMedia(match) {
  ...
  const handleSaveClick = () => {
    ...
    for (let i = 0; i < rows.length; i++) {
      let temp = {}
      let tempid = rows[i].getAttribute('customid')
      //console.log(tempid)
      temp.name = rows[i].childNodes[0].innerText
      temp.from = document.getElementById(tempid + '_fromdate').value
      temp.to = document.getElementById(tempid + '_todate').value
...
function ScheduleRow(props) {
  ...
  return (
    <tr className='schedule-row' customid={media.id}>
      <td>
        {/*<div key={media.id + to_date}>*/}
        <div>
          {media_name}
          <MdDelete
